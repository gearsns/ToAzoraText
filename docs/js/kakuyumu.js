(async function () {
    ...
    String.prototype.replaceAozraTagEscape = function(){
        return this.replace(new RegExp(`[${Object.keys(AOZORA_ESCAPE_ENTITIES).join("")}]`, "g"), m => AOZORA_ESCAPE_ENTITIES[m]);
    }
    const toAozora = text => {
        text = text.replace(/[\r\n]+/g, "")
            .replace(/<br.*?>/ig, "\n")
    ...
    const writeAozraText = async (outputFileHandle, bodyDirHandle, toc) => {
        ...
        await writable.write(`${toc.title.replaceAozraTagEscape()}\n`);
        await writable.write(`${toc.author.replaceAozraTagEscape()}\n`);
        await writable.write(`\n［＃区切り線］\n${toc.story.replaceAozraTagEscape()}\n［＃区切り線］\n\n`);
        let chapter = "";
    ...
    const getEpisodeData = async (basedir, item) => {
        ...
        return { url: url, body: getBody(doc), introduction: introduction, postscript: postscript };
    }
    ...
    const refreshNovelList = async _ => {
        ...
        elNovelListData.innerHTML = NovelList.map(item =>
            `<tr class="NovelListItem" ncode="${item.ncode}" page="${item.page}" title="${escapeHtml(item.title)}">
                ...
                <td>${item.page}
                <td><a href='${getNcodeUrl(item.ncode)}'>${escapeHtml(item.title)}</a>
